...
const FullChecklist = () => {
  ...
  const downloadAsWord = async () => {
    ...
    const doc = new Document({
      sections: [{
        ...
        children: [
          new Paragraph({
            children: [new TextRun({ text: 'CHART: Chatbot Assessment Reporting Tool', bold: true, size: 32 })],
            heading: HeadingLevel.HEADING_1,
            alignment: AlignmentType.CENTER,
          }),
          new Paragraph({
            children: [new TextRun({ text: 'Reporting Checklist for Chatbot Health Advice Studies', size: 24 })],
            alignment: AlignmentType.CENTER,
            spacing: { after: 400 },
          }),
          new Table({
          ...
          new Paragraph({
            children: [new TextRun({ text: '*If in supplementary appendix, indicate "supp" and appendix #, if applicable.', italics: true })],
            spacing: { before: 200 },
          }),
        ],
      }],
    ...
    saveAs(blob, 'chart-checklist.docx');
  };
  ...
  const downloadAsPDF = async () => {
    ...
    if (checklistRef.current) {
      ...
      pdf.save('chart-full-checklist.pdf');
    }
  };
  
  const checklistItems = [
    { section: 'Title & Abstract', items: [
      { number: '1a', label: 'Title', description: 'State that the study is assessing one or more generative AI-driven chatbots for clinical evidence or health advice.' },
      { number: '1b', label: 'Abstract/Summary', description: 'Apply a structured format, if applicable.' }
    ]},
    { section: 'Introduction', items: [
      { number: '2a', label: 'Background', description: 'State the scientific background, rationale, and healthcare context for evaluating the generative AI-driven chatbot(s), referencing relevant literature when applicable.' },
    ...
    { section: 'Methods', items: [
      ...
      { number: '3b', label: '', description: 'State whether the generative AI model(s) and chatbot(s) are open-source or closed-source/proprietary.' },
      ...
      { number: '4b', label: '', description: 'If a base model is used, cite its development in sufficient detail to identify the model.' },
      { number: '4c', label: '', description: 'If a novel base model, tuned model, or fine-tuned model is used, describe the pre- and/or postimplementation/deployment data and parameters.' },
      ...
      { number: '5b', label: '', description: 'Provide study prompts.' },
      ...
      { number: '6c', label: '', description: 'Describe whether prompts were input into separate chat session(s).' },
      { number: '6d', label: '', description: 'Provide all generative AI-driven chatbot output/responses.' },
      ...
      { number: '7b', label: '', description: 'Describe the process undertaken for generative AI-driven chatbot performance evaluation.' },
      { number: '7bi', label: '', description: 'State the number and characteristics of team members involved in performance evaluation.' },
      { number: '7bii', label: '', description: 'Provide details of any patients and public involvement during the evaluation process.' },
      { number: '7biii', label: '', description: 'State whether evaluators were blinded to the identity of the generative AI-driven chatbot(s) under assessment.' },
      ...
      { number: '9ai', label: '', description: 'Report the measures used for performance evaluation.' }
    ]},
    { section: 'Results', items: [
      { number: '10a', label: '', description: 'Report the performance evaluation undertaken including the alignment between generative AI-driven chatbot output and ground truth or reference standard using quantitative or mixed methods approaches as applicable.' },
      { number: '10b', label: '', description: 'For responses deviating from the ground truth or reference standard, state the nature of the difference(s).' },
      { number: '10c', label: '', description: 'Report the evaluation for potentially harmful, biased, or misleading responses.' }
    ]},
    { section: 'Discussion', items: [
      { number: '11a', label: '', description: 'Interpret study findings in the context of relevant evidence.' },
      { number: '11b', label: '', description: 'Describe the strengths and limitations of the study.' },
      { number: '11c', label: '', description: 'Describe the potential implications for practice, education, policy, regulation, and research.' }
    ]},
    { section: 'Open Science', items: [
      { number: '12a', label: 'Disclosures', description: 'Report any relevant conflicts of interest for all authors.' },
      { number: '12b', label: 'Funding', description: 'Report sources of funding and their role in the conduct and reporting of the study.' },
      { number: '12c', label: 'Ethics', description: 'Describe the process undertaken for ethical approval.' },
      { number: '12ci', label: '', description: 'Describe the measures taken to safeguard data privacy of patient health information, as applicable.' },
      { number: '12cii', label: '', description: 'State whether permission/licensing was obtained for the use of original, copyrighted data.' },
      { number: '12d', label: 'Protocol', description: 'Provide a study protocol.' },
      { number: '12e', label: 'Data availability', description: 'State where study data, code repository, and model parameters can be accessed.' }
    ]}
  ];
    ...
    <PageContainer>
      ...
      <ActionButtons>
        ...
        <Button 
          theme={theme} 
          variant="secondary"
          onClick={() => navigate('/fullchecklist-information')}
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <ChecklistContainer ref={checklistRef} theme={theme}>
        <ChecklistHeader>
          <h2>CHART: Chatbot Assessment Reporting Tool</h2>
          <p>Reporting Checklist for Chatbot Health Advice Studies</p>
        </ChecklistHeader>
